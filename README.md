...
Отправляйте задачи текстом, фото, **PDF и Word**. Файлы скачиваются из Telegram (или по публичной ссылке), из них извлекается текст — это передаётся Cloud Agent.
...
- **PDF и Word:** бот качает файл, разбирает его и передаёт содержимое в Cursor
...
2. Если файл больше — скачивает по публичной ссылке (Google Drive / OneDrive / Dropbox / Яндекс Диск / прямой `.pdf`/`.docx`, до 80 МБ).
...
```
1. PDF/Word до 20 МБ — просто файлом в чат (можно с подписью-задачей).
2. Файл больше 20 МБ — ссылка:
   сделай конспект https://1drv.ms/b/c/…
3. Или сначала файл/ссылка, потом текст задачи.
```

Лимиты: до 3 PDF в очереди, пароль на файле не поддерживается. Google Drive / OneDrive: доступ «любой, у кого есть ссылка».
...
word_attachments.py   — разбор Word .doc/.docx → текст + картинки
pdf_urls.py           — скачивание больших файлов по публичной ссылке
